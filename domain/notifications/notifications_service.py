    ...
    def __process_reading__(self, dict, parameter):
        ...
        if parameter_settings.min > reading:
            self.notificationRepository.add_sensor_notification(
                notification_message.format(param=parameter, direction='low', min=parameter_settings.min,
                                            max=parameter_settings.max))
        elif parameter_settings.max < reading:
            self.notificationRepository.add_sensor_notification(
                notification_message.format(param=parameter, direction='high', min=parameter_settings.min,
                                            max=parameter_settings.max))
